...
footyApp.controller('BaseController', ['$scope', '$location', '$rootScope', 'teamRunningTotals', 'UpdateScores', function($scope, $location, $rootScope, teamRunningTotals, UpdateScores) {
    // load the constants
...
//    $location.path($location.path() + 'home/');
    ...
    console.log("teamRunningTotals:" + JSON.stringify(teamRunningTotals[0]));
    ...
    var distinctColors = ['#222222', '#F3C300', '#875692', '#F38400', '#A1CAF1', '#BE0032',
                          '#C2B280', '#848482', '#008856', '#E68FAC', '#0067A5', '#F99379',
                          '#604E97', '#F6A600', '#B3446C', '#DCD300', '#882D17', '#8DB600',
                          '#654522', '#E25822', '#2B3D26']

    /**
     * Set up the data object for chart.js line chart.
     * @param {int} weeksToDisplay - the number of weeks to display
     */
    $scope.setScoreChart = function(weeksToDisplay) {

        // add data for each team to the chart data
        angular.forEach(teamRunningTotals, function(teamData, index) {
            var data = {
                label: teamData.manager,
                data: teamData.scores.slice(-weeksToDisplay),
                borderColor: distinctColors[index],
                fill: false
            }

            $scope.chartData.datasets.push(data);
        });

        // set up labels for the x-axis based on the no. of weeks to display
        var currentWeek = teamRunningTotals[0].scores.length;
        var weeks = []
        ...
        $scope.chartData.labels = weeks.slice(-weeksToDisplay);
    ...
    var defaultWeeksToDisplay = 8;
    $scope.setScoreChart(defaultWeeksToDisplay);
